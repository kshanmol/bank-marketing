...
def FitModel(train_data, train_labels, n_folds = 5, random_seed = CONST_RANDOM_SEED, max_iter = 1000, undersampled = False):
	...
	# if using LinearSVC
	# param_grid = [
	# 	{'C': map(lambda x: 2 ** x, range(-14, 4)), 'max_iter': [max_iter],
	...
	# class_weights = []
	# for x in map(lambda x: 2 ** x, range(-2, 4)):
	...
	if not undersampled:
		...
		opt_hyperparameters = {'loss': 'squared_hinge', 'C': 0.25, 'max_iter': 1000, 'random_state': 42, 'dual': False}
...
def TestModel(test_data, test_labels, scaler, model):
	...
	plot_confusion_matrix(cf_mat, classes = ['no', 'yes'], title = 'Confusion Matrix - SVM with linear kernel')
