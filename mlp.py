...
def FitModel(train_data, train_labels, cross_val_splits = 5, random_seed = CONST_RANDOM_SEED, max_iter = 1000, do_grid_search = False, retrain = False):
	...
	if do_grid_search:
		...
		clf.fit(train_data, train_labels)
		joblib.dump(clf, './models/model_1.model')
	else:
		if(retrain):
			clf = MLPClassifier(activation = 'tanh', alpha = 0.1, solver='sgd', hidden_layer_sizes=(6), learning_rate_init = 0.1, random_state = random_seed)
			clf.fit(train_data, train_labels)
			joblib.dump(clf, './models/model_1.model')
		else:
